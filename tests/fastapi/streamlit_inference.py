...
class Inference:
    ...
    def source_upload(self):
        ...
        if self.source == "video":
            ...
            vid_file = self.st.sidebar.file_uploader("Upload Image File", type=["jpeg", "png"])
            if vid_file is not None:
                g = io.BytesIO(vid_file.read())  # BytesIO Object
    ...
    def inference(self):
        ...
        if self.st.sidebar.button("Start"):
            ...
            file_path = '../../inputs/sample.mp4'
            url = "http://localhost:8000/upload-video"
            # url = "http://localhost:8000/api/video/upload"
            files = {
                'file': ('5274080-hd_1920_1080_30fps.mp4', open(file_path, 'rb'), 'video/mp4')
            }
        ...
        # Send video path to backend
            ...
            # self.st.write_stream(requests.get(url="http://localhost:8000/video",stream=True))
            # self.st.video(requests.get(url="http://localhost:8000/video",stream=True))
